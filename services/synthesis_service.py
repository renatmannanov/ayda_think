...
Выбери {target} заметок, которые НАИБОЛЕЕ РЕЛЕВАНТНЫ именно этой теме.
Не бери заметки, которые лишь косвенно связаны.
Верни ТОЛЬКО номера (id) через запятую, без пояснений.

Заметки:
{fragments_list}"""

SYNTHESIS_PROMPT = """Ты — аналитик, который находит СКРЫТЫЕ СВЯЗИ между заметками одного человека.
Тема: "{topic}".
Заметки упорядочены по дате.

Твоя задача — найти то, что автор сам мог не заметить:

🔗 СВЯЗИ — заметки из разных периодов/контекстов, которые говорят об одном и том же разными словами. Укажи какие именно и почему они связаны.

🔄 ПОВТОРЫ С РАЗВИТИЕМ — одна мысль, которая появляется несколько раз, но каждый раз конкретнее/глубже. Покажи эту цепочку.

⚡ ПРОТИВОРЕЧИЯ — где автор сам себе противоречит (возможно не осознавая). Процитируй оба фрагмента.

🌉 НЕОЖИДАННЫЕ МОСТЫ — заметка из одной темы, которая неожиданно связана с заметкой из другой.

→ ЗРЕЮЩАЯ МЫСЛЬ — если видишь идею, которая набирает силу через несколько заметок, скажи прямо: "Похоже ты движешься к [конкретный вывод]".

Формат ответа:
- Каждая найденная связь — отдельный блок с коротким заголовком
- Обязательно цитируй ключевые фразы из заметок (кратко, 5-10 слов)
- Ссылки: [#ID, дата] — например [#207, 2024-09-14]
- Если заметки не соответствуют теме "{topic}" — скажи прямо, не натягивай анализ
- НЕ пересказывай заметки. Я их уже читал. Покажи только то, что я мог не увидеть
