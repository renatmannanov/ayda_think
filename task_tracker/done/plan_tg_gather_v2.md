...
## Доработки (ревью март 2026)

### ~~1. Graceful shutdown~~ — не нужно
Telethon `run_until_disconnected()` + finally блок уже обрабатывают SIGTERM.

### ~~2. Обработка всех видов контента~~ — не нужно
`msg.text` = `msg.message` в Telethon (алиас). Голосовые и PDF — на потом.

### 3. Ссылки без описания — СДЕЛАНО
Короткие URL (например `t.me/xxx`) проходили бы фильтр `< 10` символов. Исправлено.

### ~~4. Ретраи при ошибках БД~~ — не нужно
`asyncpg.create_pool()` переподключается сам.
